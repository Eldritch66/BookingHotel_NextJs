...
export default function Card({ property }: { property: Property }) {
  ...
  return (
    <div className="bg-white col-span-1 row-span-1 rounded-2xl shadow-md grid grid-rows-2">
      ...
      <div className="row-span-1 mt-4 ml-6 mr-6 relative">
        ...
        <p className="text-orange-600 absolute bottom-4">
          <span className="font-semibold text-xl">
            {formatRupiah(property.price_per_night)}
          </span>
          <span className="text-gray-400 text-sm">/night</span>
        </p>
        <p className="absolute bottom-4 right-0 bg-black text-white p-2 rounded-xl text-sm">
          <span className="m-2">View Details</span>
        </p>
      </div>
    </div>
  );
